 ...
 */
public class SortingTopNCollector implements Collector<Row, PriorityQueue<Object[]>, Bucket> {
    ...
    public SortingTopNCollector(Collection<? extends Input<?>> inputs,
        ...
        int expectedRowCount = ((Long) numExpectedRows).intValue();
        if (expectedRowCount > 0) {
            this.initialQueueCapacity = Math.min(expectedRowCount, limit) + offset;
        } else {
            this.initialQueueCapacity = maxNumberOfRowsInQueue;
        }
        ...
        if (maxNumberOfRowsInQueue >= ArrayUtil.MAX_ARRAY_LENGTH || maxNumberOfRowsInQueue < 0) {
            ...
            // 2) if offset + limit exceeds Integer.MAX_VALUE (then initialQueueCapacity is negative!)
            throw new IllegalArgumentException(
                "Invalid LIMIT + OFFSET: value must be <= " + (ArrayUtil.MAX_ARRAY_LENGTH - 1) + "; got: " +
                maxNumberOfRowsInQueue);
        }
    }

    @Override
    public Supplier<PriorityQueue<Object[]>> supplier() {
        return () -> new PriorityQueue<>(initialQueueCapacity, comparator);
    }
    ...
    private void onNextRow(PriorityQueue<Object[]> pq, Row row) {
        ...
        if (pq.size() == maxNumberOfRowsInQueue) {
            ...
            if (comparator.compare(rowCells, highestElementInOrder) > 0) {
                pq.poll();
                pq.add(rowCells);
            }
        } else {
